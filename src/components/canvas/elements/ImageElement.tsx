...
import { Group, Rect as KonvaRect, Image as KonvaImage } from 'react-konva';
import type { ImageElement as ImageElementType, CustomElement, ShadowStyle } from '../../../types';
...
export const ImageElement: React.FC<ImageElementProps> = ({
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      console.error('图片加载失败:', element.src?.slice(0, 50));
      setImage(null);
      setImageLoaded(false);
      setLoadError(true);
    };
  ...
  return (
    ...
    >
      ...
      <KonvaRect
        ...
        fill={loadError ? '#FEE2E2' : (!imageLoaded ? '#F3F4F6' : 'transparent')}
        stroke={loadError ? '#F87171' : '#E5E7EB'}
        strokeWidth={imageLoaded ? 0 : 2}
